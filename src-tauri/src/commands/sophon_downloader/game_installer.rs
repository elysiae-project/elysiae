...
pub async fn build_installers(
    ...
    let branch_resp: FrontDoorResponse = client.get(FRONT_DOOR_URL).send().await?.json().await?;
    ...
    let build_url = format!(
        ...
        SOPHON_BUILD_URL_BASE, branch.main.branch, branch.main.package_id, branch.main.password,
    );

    let build_resp: SophonBuildResponse = client.get(&build_url).send().await?.json().await?;
    ...
    let game_installer =
        SophonInstaller::from_manifest_meta(client, &manifests[0], temp_dir).await?;
    ...
    let vo_installer = SophonInstaller::from_manifest_meta(client, vo_meta, temp_dir).await?;
...
impl SophonInstaller {
    pub async fn from_manifest_meta(
        ...
        // Directory creation is fast; keep it synchronous via spawn_blocking to
        // avoid blocking the executor thread.
        let inst_temp_clone = inst_temp.clone();
        tokio::task::spawn_blocking(move || fs::create_dir_all(inst_temp_clone.join("chunks")))
            .await??;
    ...
    pub async fn install(
        ...
        for chunk in &unique_chunks {
            ...
            let already_done = tokio::task::spawn_blocking(move || {
                dest_clone.exists() && check_file_md5(&dest_clone, chunk_size, &expected_md5)
            })
            .await
            .map_err(|e| e.to_string())?;

            if already_done {
                downloaded_bytes += chunk.chunk_size;
        ...
        for (idx, file) in files.iter().enumerate() {
            ...
            tokio::task::spawn_blocking(move || {
                assemble_file(&file, &game_dir, &chunks_dir, &temp_dir).map_err(|e| {
